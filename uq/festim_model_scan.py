...
def make_parameter_values_list(param_def_val, level_variation=3, scale="log"):
    ...
    if scale == "log":
        ...
        vary_factor = 0.1
        linear_scale_range = (
            level_variation * vary_factor * param_def_val
        )  # try level_variation * 10% of the default value towards higher and lower values
...
def parameter_scan(config, param_name="length", level_variation=3, target_dir="./", scale="log"):
    ...
    for value in param_values:
        ...
        config[param_name] = (
            value  # TODO make fall back if config does not have this parameter or the structure is different
        )
    ...
    if scale == "log":
        r_scale_slope = (np.log10(results_toplot[-1]) - np.log10(results_toplot[0])) / (
            np.log10(param_values[-1]) - np.log10(param_values[0])
        )
    # Compute the lin-log slope of the scale
    elif scale == "linear":
        r_scale_slope = (np.log10(results_toplot[-1]) - np.log10(results_toplot[0])) / (
            param_values[-1] - param_values[0]
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--parameter", "-p", default="length", help="Parameter which to vary during the scan (default: length))"
    )

    parser.add_argument("--targetdir", "-d", default="./", help="Target directory where to save the results of runs")

    parser.add_argument("--scale", "-s", default="log", help="Scale to apply to the varying parameter")
    ...
    if config:
        ...
        if scan_type == "single":
            ...
            parameter_scan(
                config, param_name=param_name, level_variation=level_variation, target_dir=target_dir, scale=scale
            )
